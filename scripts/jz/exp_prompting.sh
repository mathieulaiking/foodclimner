...
models=(
    "Qwen/Qwen3.5-27B|a100|1"
    "google/gemma-4-31B-it|a100|1"
    "Qwen/Qwen3.5-0.8B|a100|1"
    "Qwen/Qwen3.5-9B|a100|1"
)
...
for model_entry in "${models[@]}"; do
    ...
    for dataset in "${datasets[@]}"; do
        ...
        output_dir="out/experiments/prompting/${model_short}/${dataset_short}"

        job_script="${GENERATED_DIR}/run_prompting_${model_short}_${dataset_short}.slurm"
        ...
        common_render_template \
            "${TEMPLATE}" \
            "${job_script}" \
            "__JOB_NAME__=prompting-${model_short}-${dataset_short}" \
            "__LOG_PREFIX__=prompting-${model_short}-${dataset_short}" \
            "__GPU_NB__=${gpu_nb}" \
        ...
        common_submit_job "${job_script}" "prompting model=${model_short} dataset=${dataset_short}"
